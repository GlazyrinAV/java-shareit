...
public class FindByOwnerRejected implements StrategyByOwner {
    ...
    public Collection<BookingDto> findByBookingState(int userId, Integer from, Integer size) {
        if (PageCheck.isWithoutPage(from, size)) {
            return bookingMapper.toDto(bookingRepository.findTop100ByItem_Owner_IdAndStatusEqualsOrderByStartDesc(userId, BookingStatus.REJECTED));
        }
        Pageable page = PageRequest.of(from == 0 ? 0 : from / size, size, Sort.by("start").descending());
        return bookingMapper.toDto(bookingRepository.findByItem_Owner_IdAndStatusEquals(userId, BookingStatus.REJECTED, page).getContent());
    }
