...
public class FindByOwnerWaiting implements StrategyByOwner {
    ...
    public Collection<BookingDto> findByBookingState(int userId, Integer from, Integer size) {
        if (from == null || size == null) {
            return bookingMapper.toDto(bookingRepository.findByItem_Owner_IdAndStatusEqualsOrderByStartDesc(userId, BookingStatus.WAITING));
        }
        if (from < 0 || size < 1) {
            throw new WrongParameter("Указаны неправильные параметры.");
        }
        Pageable page = PageRequest.of(from == 0 ? 0 : from / size, size);
        return bookingMapper.toDto(bookingRepository.findByItem_Owner_IdAndStatusEqualsOrderByStartDesc(userId, BookingStatus.WAITING, page).getContent());
    }
