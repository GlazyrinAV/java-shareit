...
public class FindByStateAll implements StrategyByState {
    ...
    public Collection<BookingDto> findByBookingState(int userId, Integer from, Integer size) {
        if (from == null || size == null) {
            return bookingMapper.toDto(bookingRepository.findByBooker_IdEqualsOrderByStartDesc(userId));
        }
        if (from < 0 || size < 1) {
            throw new WrongParameter("Указаны неправильные параметры.");
        }
        Pageable page = PageRequest.of(from == 0 ? 0 : from/size, size);
        return bookingMapper.toDto(bookingRepository.findByBooker_IdEqualsOrderByStartDesc(userId, page).getContent());
    }
