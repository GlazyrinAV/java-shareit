...
public class FindByStatePast implements StrategyByState {
    ...
    public Collection<BookingDto> findByBookingState(int userId, Integer from, Integer size) {
        if (PageCheck.isWithoutPage(from, size)) {
            return bookingMapper.toDto(bookingRepository.findTop100ByBooker_IdAndEndBeforeOrderByStartDesc(userId, LocalDateTime.now()));
        }
        Pageable page = PageRequest.of(from == 0 ? 0 : from / size, size, Sort.by("start").descending());
        return bookingMapper.toDto(bookingRepository.findByBooker_IdAndEndBefore(userId, LocalDateTime.now(), page).getContent());
    }
