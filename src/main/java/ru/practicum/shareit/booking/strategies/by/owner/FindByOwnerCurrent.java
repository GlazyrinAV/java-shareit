...
public class FindByOwnerCurrent implements StrategyByOwner {
    ...
    public Collection<BookingDto> findByBookingState(int userId, Integer from, Integer size) {
        ...
        return bookingMapper.toDto(bookingRepository.findByItem_Owner_IdAndStartBeforeAndEndAfterOrderByStartDesc(userId, LocalDateTime.now(), LocalDateTime.now(), page).getContent());
    }
