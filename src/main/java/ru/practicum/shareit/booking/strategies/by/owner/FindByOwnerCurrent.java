...
public class FindByOwnerCurrent implements StrategyByOwner {
    ...
    public Collection<BookingDto> findByBookingState(int userId, Integer from, Integer size) {
        if (PageCheck.isWithoutPage(from, size)) {
            return bookingMapper.toDto(bookingRepository.findTop100ByItem_Owner_IdAndStartBeforeAndEndAfterOrderByStartDesc(userId, LocalDateTime.now(), LocalDateTime.now()));
        }
        Pageable page = PageRequest.of(from == 0 ? 0 : from / size, size, Sort.by("start").descending());
        return bookingMapper.toDto(bookingRepository.findByItem_Owner_IdAndStartBeforeAndEndAfter(userId, LocalDateTime.now(), LocalDateTime.now(), page).getContent());
    }
